#!/usr/bin/env python3
"""Compile and run CDC wrapper asynchronous integration co-simulation."""
...
REPORT_JSON = PROJECT_ROOT / "reports" / "cdc_cosim_summary.json"
REPORT_MD = PROJECT_ROOT / "reports" / "cdc_cosim_summary.md"
REPORT_LOG = PROJECT_ROOT / "reports" / "cdc_cosim.log"
...
def write_summary(
    *,
    ...
    lines: list[str] = [
        "# CDC Wrapper Async Co-Simulation Summary",
        "",
...
def main() -> int:
    compile_cmd = [
        ...
        "-Wall",
        "-o",
        "sim/build/tb_cdc_wrapper_async.vvp",
        "sim/tb_cdc_wrapper_async.sv",
        "rtl/waveform_brain_cdc_wrapper.v",
        "rtl/xpm_cdc_stubs.v",
    ]
    run_cmd = ["vvp", "sim/build/tb_cdc_wrapper_async.vvp"]
    ...
    out = build_dir / "tb_cdc_wrapper_async.vvp"
    ...
    passed = (
        ...
        and "TB_PASS tb_cdc_wrapper_async" in run_proc.stdout
    )
    write_summary(
        passed=passed,
